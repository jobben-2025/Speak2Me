...
class Animation:
    def __init__(self,name, image_id, frames, fps=10.0, fps_min=1):
        self.image_id = image_id
        self.name = name
        self.frames = frames
        self.frame_iterator = cycle(frames)
        self.fps = fps              # the actual current fps
        self.fps_max = fps          # what fps recovers to using lerp
        self.fps_min = fps_min      # what ani drops to on GUI changes
        self.lerp_factor = 0.005    # how fast fps recovers
    ...
    @property
    def spf_lerp(self): #seconds per frame
        dt = time.time() - self.last_update
        dt = min(dt,0.1) # avoid jumps
        self.fps = lerp(self.fps, self.fps_max, dt*self.lerp_factor)
        return 1.0 / self.fps # spf
    
    def slow_down(self):
        self.fps = self.fps_min
...
class AnimatedCanvas(tk.Canvas):
    def __init__(self, *args, **kwargs):
        ...
        self.running = True
        self.exit = False
        self.fps = 1/20 # not used
        self.thread = Thread(target=self.update_background, daemon=True)
        self.lock = Lock()
        self.after_method = self.master.after_idle
        self.paused_on = float()
    
    def create_animation(self,*args,image:Image.Image, name=None, fps=10.0, **kwargs):
        img_frames = []
            ...
            if self.running:
                ...
                with self.lock:
                    for img_id,ani in self.animations.items():
                        if ani.running and now - ani.last_update >= ani.spf_lerp:
...
class App:
    ...
    def __init__(self, root:Tk, asset_paths:dict):
        ...
        self.bakani_id,_ = self.bakani.create_animation(0, 0, anchor='nw', name='backani', fps=60,
                                                      image=self.assets_raw.get(ASSET_BACKGROUND, self.FB_IMAGE))
        self.bakani.pack(fill='both',expand=True)
    ...
    def on_configure(self,event:tk.Event):
        self.bakani.animations[self.bakani_id].slow_down()
        self.bakani.pause_animation()
        self.root.after_idle(self.bakani.unpause_animation)
    ...
    def on_key_pressed(self,event): print(f'key: {getattr(event,'keysym','no_attr')}')
