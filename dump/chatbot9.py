...
class ImageUtils:
    ...
    @staticmethod
    def frames_to_Img(frames) -> list[Image.Image]:
        images = []
        for f in frames:
...
class Animation:
    def __init__(self, root:tk.Tk, images:list[Image.Image]):
        ...
        self.ph_iter = cycle(self.photos)
...
class ChatbotApp:
    def __init__(self, root:tk.Tk, bot:Talk2MeBot):
        ...
        self.bg_image_path = self.path_assets / 'chatbot_background.gif'
    ...
    def load_animations(self):
        ...
        #geopatra_clips = ['neutral_v1.mp4','neutral_v2.mp4']
        for clip in geopatra_clips:
            path = self.path_assets / 'geopatra' / clip
            ani = Animation(self.root,IU.frames_to_Img(IU.extract_nd_frames(str(path))))
            name = clip[:-4]
    ...
    def on_key_press(self, event:Event):
        ...
        match getattr(event,'keysym',''): 
            ...
            case 'Left':
                ...
                self.animator.select_animation(rankey)
    ...
    def background_scroll(self):
        if self.bg_cid is not None:
            ...
            self.canvas.coords(self.bg_cid, x0, y0+20)
